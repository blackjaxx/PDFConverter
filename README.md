...
> 每次推送 `v*` 标签（如 `v0.3.21`）时，GitHub Actions 会自动构建并上传 DMG 包。所有 CLI 工具（Poppler、qpdf、Ghostscript、Tesseract）已打包在 App Bundle 内，无需额外安装 Homebrew。

### 本地构建

```bash
# 1. 安装开发依赖（CI 构建时自动打包，本地调试需要）
brew install poppler qpdf ghostscript tesseract

# 2. 打包 CLI 工具到 App Bundle
...
┌────────────────────▼─────────────────────────────┐
│   系统工具层                                       │
│   PDFKit（macOS 原生）+ 离线捆绑 CLI 工具（Poppler 等）│
└──────────────────────────────────────────────────┘
```
...
`ProcessRunner` 使用 `readabilityHandler` 实时读取子进程的 stdout/stderr 管道数据，防止大型输出（>64KB）时管道缓冲区溢出导致进程死锁。自动设置 `DYLD_FALLBACK_LIBRARY_PATH` 确保子进程能找到捆绑的 dylib 依赖。
...
App 将以下 CLI 工具及其动态库依赖打包在 `Contents/Resources/tools/` 中，实现真正的零网络依赖运行：

| 目录 | 工具 | 用途 |
|------|------|------|
| `tools/poppler/` | pdftoppm, pdftotext | PDF 转图片/文本 |
| `tools/qpdf/` | qpdf | 加密/解密/拆分 |
| `tools/ghostscript/` | gs | 压缩/优化 |
| `tools/tesseract/` | tesseract + tessdata | OCR 识别 |

打包脚本 `Scripts/bundle-tools.sh` 负责：
- 从 Homebrew 复制二进制文件
- 解析并捆绑所有 `.dylib` 依赖（包括 `@rpath` 引用）
- 修改 `install_name` 为 `@executable_path`，确保在任意 Mac 上可运行
- 多轮后处理扫描，修复残留的动态库引用
...
1. 安装 Homebrew 依赖（poppler、qpdf、ghostscript、tesseract）
2. 运行 `bundle-tools.sh` 打包 CLI 工具和 dylib 到 App Bundle
3. `xcodebuild build` 编译 Release 版本
4. 验证打包的工具是否可运行（含 `otool -L` 诊断信息）
5. 生成 `.dmg` 安装包
